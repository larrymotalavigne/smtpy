...
from back.core.config import SETTINGS, ALEMBIC_CONFIG
...
if SETTINGS.DATABASE_URL:
    ...
    logging.info(f"Database URL configuration: Using {db_type} database")
    logging.debug(f"Sync database URL: {DATABASE_URL}")
    logging.debug(f"Async database URL: {ASYNC_DATABASE_URL}")
else:
    DB_PATH = SETTINGS.DB_PATH
    DATABASE_URL = f"sqlite:///{DB_PATH}"
    ASYNC_DATABASE_URL = f"sqlite+aiosqlite:///{DB_PATH}"
    
    logging.info(f"Database URL configuration: Using SQLite database (default)")
    logging.debug(f"SQLite path: {DB_PATH}")
    logging.debug(f"Sync database URL: {DATABASE_URL}")
    logging.debug(f"Async database URL: {ASYNC_DATABASE_URL}")
