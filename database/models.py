...
class User(Base):
    ...
    invitations: Mapped[List["Invitation"]] = relationship("Invitation", back_populates="inviter", cascade="delete")
    domains: Mapped[List["Domain"]] = relationship("Domain", back_populates="owner", cascade="delete")
...
class Domain(Base):
    ...
    catch_all: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # Email address for catch-all alias (if set)
    ...
    aliases: Mapped[List["Alias"]] = relationship("Alias", back_populates="domain", cascade="delete")
    forwarding_rules: Mapped[List["ForwardingRule"]] = relationship("ForwardingRule", back_populates="domain", cascade="delete")
...
class Alias(Base):
    ...
    expires_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, nullable=True, index=True)
