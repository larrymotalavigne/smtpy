# Page snapshot

```yaml
- generic [ref=e3]:
  - generic [ref=e5]:
    - generic [ref=e6]:
      - heading "SMTPy" [level=1] [ref=e8]
      - heading "Connexion" [level=2] [ref=e9]
      - paragraph [ref=e10]: Connectez-vous à votre compte
    - generic [ref=e14]:
      - alert [ref=e16]:
        - generic [ref=e18]: Une erreur est survenue lors de la connexion
      - generic [ref=e19]:
        - generic [ref=e20]: Nom d'utilisateur ou email *
        - generic [ref=e21]:
          - generic: 
          - textbox "Nom d'utilisateur ou email *" [ref=e22]:
            - /placeholder: Entrez votre nom d'utilisateur
            - text: invalid@example.com
      - generic [ref=e23]:
        - generic [ref=e24]: Mot de passe *
        - generic [ref=e25]:
          - generic: 
          - generic [ref=e26]:
            - textbox "Entrez votre mot de passe" [ref=e27]: wrongpassword
            - img [ref=e28]
      - generic [ref=e30]:
        - checkbox [ref=e33] [cursor=pointer]
        - link "Mot de passe oublié?" [ref=e35] [cursor=pointer]:
          - /url: /auth/forgot-password
      - button " Se connecter" [ref=e37] [cursor=pointer]:
        - generic [ref=e38]: 
        - generic [ref=e39]: Se connecter
      - generic [ref=e41]: ou
      - paragraph [ref=e43]:
        - text: Vous n'avez pas de compte?
        - link "Créer un compte" [ref=e44] [cursor=pointer]:
          - /url: /auth/register
    - link " Retour à l'accueil" [ref=e46] [cursor=pointer]:
      - /url: /
      - generic [ref=e47]: 
      - text: Retour à l'accueil
  - alert [ref=e50]:
    - generic [ref=e51]:
      - img [ref=e52]
      - generic [ref=e55]:
        - generic [ref=e56]: Erreur de connexion
        - generic [ref=e57]: Une erreur est survenue lors de la connexion
      - button "Close" [active] [ref=e59] [cursor=pointer]:
        - img [ref=e60]
```